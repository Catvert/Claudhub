...
> **Tous les jalons sont faits.** Ce document est gardé pour ce qu'il explique
> — le raisonnement derrière chaque décision — mais les cases sont cochées et
> `CLAUDE.md` fait désormais foi sur la structure. Les références en
> `chemin:ligne` datent du 2026-08-19 et ont bougé depuis.
...
- [x] `src/ui/store.rs`, calqué sur `src/ui/settings.rs` : un global gpui, un
...
- [x] Y persister `ReviewState::base` — l'état vivant reste en mémoire, seule
...
- [x] Y persister `collapsed` (repli des dossiers de la liste de revue).
- [x] Purger à l'ouverture d'un dépôt les entrées dont le worktree n'existe
      plus, en croisant avec `Repo::worktrees()`.
- [x] `CLAUDE.md` : assumer que ce fichier est écrit **depuis le thread
...
- [x] Le modèle, dans `src/ui/notes.rs` :
...
- [x] `notes::relocate(&Rendered, &Note) -> Anchor` — **fonction libre et
...
- [x] Bouton dans l'en-tête de la vue diff, à côté de `copy-file` (`:717`).
- [x] **Panneau `ClaudhubNotes`** (« Relecture ») : les notes du worktree, groupées
...
- [x] `notes::prompt(&[Note]) -> String` — **libre et testée**, c'est la pièce à
...
- [x] `TerminalGroup::send_to_agent(text)` : écrit dans le pty de l'onglet agent
...
- [x] **Le `\r` final part séparément**, après le collage, et non dans le même
      paquet : un TUI qui vient de recevoir un collage encadré peut avaler un
      retour chariot collé au bout.
- [x] Vérifier le comportement sur plusieurs milliers de caractères.
- [x] Trois boutons : « envoyer les notes non traitées », « envoyer cette
...
- [x] Les notes envoyées passent à `sent`, pas à `done` : c'est la relecture de
...
- [x] `Settings::terminal.agent_command: String` (`src/ui/settings.rs:127`)
...
- [x] **Migration silencieuse** : si `agent_command` est présent et `agents`
      vide, en faire un profil. `#[serde(default)]` (`settings.rs:116`, `:168`)
      rend l'opération sans risque.
- [x] **Correctif au passage** : `open_agent` découpe la commande par
...
- [x] `Cmd::ScanAgents { program }` (`src/runtime/protocol.rs:55`) prend
      ...
      combien.
- [x] Le menu « + » du groupe de terminaux (`terminal_view.rs:908`) liste les
...
- [x] Dans `/home/finch/Projets/wt` : ajouter `src/lib.rs` exportant `config`,
...
- [x] Claudhub en dépend (`path` pendant le développement, `git =` ensuite).
      `src/wt.rs` côté Claudhub, couche métier sans gpui, lit le `wt.toml` du dépôt.
- [x] **Tout appel à `ops::App` part dans un worker** : il lance des hooks shell
...
- [x] **Création guidée** : le dialogue de nouveau worktree
...
- [x] **Menu contextuel d'un worktree** : `up`, `down`, `rm`, et **les
      `[tasks.*]` du projet, listées dynamiquement**.
- [x] **Exécution dans un onglet terminal**, pas dans un panneau de sortie : les
...
- [x] **Statut et URLs** : `[status] up` et `[open]` alimentent la barre latérale
...
- [x] `git/repo.rs` : `merge`, `rebase`, `merge_abort`, `merge_continue`, et les
      `Cmd`/`Evt`/`Action` correspondants.
- [x] **« Mettre à jour depuis la base »** dans le worktree — la base a avancé
      pendant que l'agent travaillait. Merge ou rebase selon un réglage.
- [x] **« Intégrer dans `<base>` »** — exécuté **depuis le dépôt principal**,
...
- [x] **Conflits** : `Status` connaît déjà l'état `Unmerged`
...
- [x] **Garde-fou** : un merge interrompu laisse le dépôt à mi-chemin. Tant
...
- [x] `git ls-files --cached --others --exclude-standard` donne en **un seul
...
- [x] Bascule « montrer aussi les fichiers ignorés », qui demande elle une
      lecture de disque paresseuse (`Cmd::ListDir { path }`, un niveau à la
      fois, depuis le worker) — en second temps.
- [x] **Généraliser `review::tree_rows`** (`src/ui/review.rs:1021`) plutôt
...
- [x] Panneau `ClaudhubFiles`. Bonus quasi gratuit une fois l'arbre généralisé :
      afficher le statut git, puisque `ReviewState::status` est déjà là.
- [x] Actions : ouvrir, renommer, supprimer, nouveau fichier/dossier, copier le
      chemin. Confirmation pour les destructions — modèle : `confirm_removal`
      (`review.rs:487`).

### Lire et retoucher

- [x] `Cmd::ReadFile { worktree, path }` → `Evt::FileContent`, et
      `Cmd::WriteFile { worktree, path, content, expect: Option<Hash> }`.
- [x] La vue utilise **`InputState::code_editor(language).line_number(true)`** de
      ...
      main.
- [x] Au-delà de la limite documentée, refuser d'ouvrir et proposer l'éditeur
      externe, plutôt que de figer la fenêtre. Détecter et refuser les binaires
      (cf. `FileDiff::binary`, `src/git/diff.rs:106`).
- [x] **Écriture concurrente** : un agent écrit dans les mêmes fichiers pendant
...
- [x] Le geste doit exister **depuis une ligne de diff**, avec son numéro de
...
- [x] **HTTP** : déclarer `ureq` (bloquant, `rustls`, pas de tokio) — il épouse
...
- [x] **Configuration** : `sentry.org` dans les réglages ; le **projet dépend du
...
- [x] `Cmd::LoadIssues { org, project, query }` → `Evt::Issues`, sur la **file
